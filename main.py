...
# ═══════════════════════════════════════════════════════════════════════
# AI 에이전트 시스템 (3에이전트: 분류 → 깊은 분석 → 종합)
# ═══════════════════════════════════════════════════════════════════════
# Agent 1 (Classification): 보유+더블컨펌을 4그룹 분류 + 시장 컨텍스트 한 줄
# Agent 2 (Deep Analysis):  매수후보/매도검토/보유주의 그룹별 깊은 분석 + 웹검색
# Agent 3 (Summary):        부장 종합 마크다운 보고서 (우선순위 액션 리스트)
# ═══════════════════════════════════════════════════════════════════════
...
AGENT_MODEL_TEAM     = "claude-sonnet-4-6"  # Agent 1, 2 기본
AGENT_MODEL_SUMMARY  = os.environ.get("AGENT_MODEL_SUMMARY", "claude-sonnet-4-6")  # Agent 3 (Opus 옵션)
AGENT_MAX_TOKENS_CLASS   = 2000
AGENT_MAX_TOKENS_DEEP    = 4000
AGENT_MAX_TOKENS_SUMMARY = 2500
...
# ─── stage 변환 헬퍼 (중앙화) ──────────────────────────────────────────
_STAGE_MAP = {1: "적극매수", 2: "매수우호", 3: "중립관망", 4: "매수축소", 5: "현금보유"}
_STAGE_NAME_TO_NUM = {v: k for k, v in _STAGE_MAP.items()}

def _score_to_stage(score):
    """매크로 점수 → (stage_num, signal_name). None 안전."""
    if score is None: return None, None
    try:
        s = int(score)
        if   s >= 48: return 1, _STAGE_MAP[1]
        elif s >= 36: return 2, _STAGE_MAP[2]
        elif s >= 24: return 3, _STAGE_MAP[3]
        elif s >= 12: return 4, _STAGE_MAP[4]
        else:         return 5, _STAGE_MAP[5]
    except: return None, None

# ─── DB 테이블 초기화 (v2 구조) ────────────────────────────────────────
def init_agent_db():
    """기존 v1 테이블을 legacy로 RENAME 후 v2 스키마 생성"""
    if not DATABASE_URL: return
    try:
        conn = get_conn(); cur = conn.cursor()
        # v1 흔적이 있으면 legacy로 RENAME (1회만)
        cur.execute("""
            DO $$
            BEGIN
                IF EXISTS (
                    SELECT 1 FROM information_schema.columns
                    WHERE table_name='agent_analysis_cache' AND column_name='macro_result'
                ) THEN
                    ALTER TABLE agent_analysis_cache RENAME TO agent_analysis_cache_legacy;
                END IF;
            END $$;
        """)
        cur.execute("""
        ...
        """)
        conn.commit(); cur.close(); conn.close()
        logger.info("agent_analysis_cache (v2) 테이블 준비됨")
    except Exception as e:
...
CLASSIFICATION_PROMPT = """당신은 WISEMAC STOCK의 분류 에이전트입니다.

## 역할 (단순 + 빠름)
1. 시장 환경을 한 줄로 요약 (다른 탭의 점수/stage 그대로 신뢰)
2. 종목을 4그룹으로 분류 (객관적 룰 기반)
3. 보유 정상 종목에 짧은 한 줄 코멘트 부여

## 절대 원칙
- 매크로 점수/stage를 재해석하지 않는다. 다른 탭이 권위.
- 종목 분류는 명시된 임계값으로만. 주관 판단 금지.
- 깊은 분석은 후속 에이전트가 하므로 여기서 깊이 들어가지 않는다.

## 분류 기준 (보수적)
- buy_candidates (최대 3): 더블컨펌 combined_score 상위 3개
- sell_review: pnl_pct ≤ -12% OR pnl_pct ≥ 45%
- holding_caution (최대 2): 위 조건 외 -5%~-12% 또는 모멘텀 점수 < 60
- holding_normal: 나머지 보유 종목 (한 줄 코멘트만)

## 입력 데이터 형식
{
  "macro": {"score": int, "stage": int, "signal_name": str},
  "flow":  {"leading_sectors": [str], "lagging_sectors": [str]},
  "holdings": [{ticker, name, sector, pnl_pct, weight_pct, ...}],
  "double_confirm": [{ticker, name, combined_score, momentum_score, total_score, ...}],
  "rules": {"stop_loss_pct": -15, "take_profit_pct": 50}
}

## 출력 JSON (이 형식 정확히 준수)
{
  "market_context": "Stage 3 중립관망 · 반도체 강세 지속 · AI 인프라 자금 유입",
  "market_color": "blue|yellow|red",
  "deep_analysis": {
    "buy_candidates": [
      {"ticker": "MU", "reason": "더블컨펌 160점 최상위"}
    ],
    "sell_review": [
      {"ticker": "RIVN", "reason": "PnL -13.5%, -15% 손절선 임박"}
    ],
    "holding_caution": [
      {"ticker": "MRVL", "reason": "모멘텀 점수 58점으로 약화"}
    ]
  },
  "shallow_check": {
    "holding_normal": [
      {"ticker": "GOOGL", "pnl_pct": 8.2, "note": "정상 보유"},
      {"ticker": "AAPL", "pnl_pct": 15.4, "note": "정상 보유"}
    ]
  }
}

## 응답 규칙
- 반드시 위 JSON 구조만 반환 (마크다운/설명 없이)
- market_context는 한 줄(60자 이내)
- market_color: stage 1~2는 "blue", 3은 "yellow", 4~5는 "red"
- 각 reason은 30자 이내, 객관적 근거만
- holding_normal의 note는 "정상 보유" 형식
"""

DEEP_ANALYSIS_PROMPT = """당신은 WISEMAC STOCK의 종목 분석 에이전트입니다.

## 역할
Agent 1이 분류한 종목 그룹을 받아 그룹 특성에 맞게 심층 분석합니다.
실전매매에 바로 쓸 수 있는 액션 신호를 제공해야 합니다.

## 절대 원칙
- 종목별로 명확한 action을 출력한다 (애매한 "관망" 금지)
- trigger는 구체적 가격/지표로 명시 (예: "MA50 하향 돌파", "진입가 -10%")
- 어닝/뉴스는 반드시 검색해 최신 정보 확보
- 입력에 없는 종목은 절대 분석하지 않는다
- 검색 결과 출처는 sources에 명시

## 그룹별 분석 포커스

### buy_candidates (매수 후보)
- 어닝 결과 + 가이던스 확인 (검색 1~2회)
- 최근 뉴스 호재/악재 확인
- 진입가 트리거 + 손절 트리거 명시
- 권장 비중 (1~5%)
- action: "분할매수" | "관심종목 추가" | "보류"

### sell_review (매도 검토)
- -15% 손절 또는 +50% 익절 룰 위반 여부
- 최근 종목 악재/추세 전환 검색 (1회)
- 매도 시점 권고 (즉시 / 분할 / 대기)
- action: "즉시매도" | "분할매도" | "유지(룰 위반 아님)"

### holding_caution (보유 주의)
- 모멘텀 약화 원인 검색 (1회)
- 손절 트리거 재확인
- 비중 축소 vs 유지 판단
- action: "비중축소" | "관찰 유지" | "분할매수 추가"

## 입력 데이터
{
  "groups": {
    "buy_candidates":  [{ticker, name, sector, combined_score, ...}],
    "sell_review":     [{ticker, name, pnl_pct, weight_pct, ...}],
    "holding_caution": [{ticker, name, pnl_pct, weight_pct, ...}]
  },
  "market_context": "Stage 3 ...",
  "rules": {"stop_loss_pct": -15, "take_profit_pct": 50}
}
...
## 출력 JSON
{
  "analyses": [
    {
      "ticker": "MU",
      "group": "buy_candidates",
      "action": "분할매수",
      "weight_suggestion": "3%",
      "headline": "어닝 가이던스 상향, 더블컨펌 최상위",
      "reasoning": "FY25 매출 가이던스 +15% 상향 (12/18 어닝). 모멘텀+펀더멘탈 동시 신호로 더블컨펌 160점 1위.",
      "trigger_buy": "현재가 ±2% 내 3회 분할",
      "trigger_sell": "MA50 하향 돌파 또는 진입가 -10%",
      "next_event": "FY25 Q2 어닝 2026-03-15 예상",
      "risks": ["반도체 사이클 피크 우려"],
      "sources": ["MU 12/18 어닝 콜", "WSJ 2026-01-XX"]
    }
  ]
}

## 응답 규칙
- 입력 groups의 모든 종목을 빠짐없이 분석
- reasoning은 80~150자 (간결 + 구체)
- trigger는 측정 가능한 지표만 (감정적 표현 금지)
- sources는 검색에서 사용한 자료명 (없으면 빈 배열)
- 마크다운/설명 없이 JSON만 반환
"""

SUMMARY_PROMPT = """당신은 WISEMAC STOCK의 종합 보고 에이전트(부장)입니다.

## 역할
Agent 1(시장 분류)과 Agent 2(종목 분석)의 결과를 받아
사용자가 5분만 봐도 오늘 무엇을 해야 할지 명확한 마크다운 보고서를 작성합니다.

## 절대 원칙
- 마크다운 형식만 (JSON 금지)
- 우선순위 정렬: 즉시매도 > 분할매도 > 분할매수 > 비중축소 > 관찰
- 종목 추천은 반드시 Agent 2의 데이터를 인용 (지어내기 금지)
- 사용자가 보유하지 않은 종목의 매도 권고 금지
- "관망", "지켜보자" 같은 공허한 표현 금지 — 측정 가능한 트리거만

## 입력 데이터
{
  "market_context": "...",
  "market_color":   "...",
  "shallow_check":  {"holding_normal": [...]},
  "analyses":       [{ticker, group, action, ...}],
  "portfolio_summary": {...},
  "rules": {"stop_loss_pct": -15, "take_profit_pct": 50}
}

## 출력 형식 (마크다운, 아래 헤딩 구조 정확히 준수)

# 🎯 오늘의 액션 리포트

## 시장 컨텍스트
(market_context를 한 문단으로 풀어쓰기)

## 🔴 즉시 검토 (해당 있을 때만)
- **TICKER (회사명)**: action — 핵심 근거
  - 트리거: trigger_sell
  - 다음 이벤트: next_event

## 🟢 신규 매수 후보 (해당 있을 때만)
- **TICKER (회사명)**: 분할매수 weight_suggestion
  - 근거: reasoning 압축 (1줄)
  - 진입: trigger_buy
  - 손절: trigger_sell
...
## 🔵 정상 보유 (N종목)
GOOGL +8.2%, AAPL +15.4%, ... (한 줄 나열)

## 종합 판단
2~3문장. 시장 환경과 오늘 액션의 연결 + 핵심 행동 한 줄.

## 응답 규칙
- 위 헤딩 구조 정확히 준수
- 섹션이 비면 "(해당 없음)" 또는 생략
- 종목 형식: **TICKER (한글명)**: action — 근거
- 전체 길이 600~800자 (간결성)
- 검색 사용 안 함
"""
...
def _collect_market_data():
    ...
    sectors = sm.get("results", []) if isinstance(sm, dict) else []
    leading = [s.get("sector") for s in sectors if s.get("trend") in ("강한유입", "유입")][:3]
    lagging = [s.get("sector") for s in sectors if s.get("trend") in ("강한유출", "유출")][:3]
    ...
    return {
        ...
        "sentiment": {
            "vix": mkt.get("vix"),
            "fear_greed": fg.get("fear_greed", {}).get("now") if isinstance(fg, dict) else None,
        }
    }
...
# ═══════════════════════════════════════════════════════════════════════
# 에이전트 호출 (프롬프트 캐싱 + 에러 처리)
# ═══════════════════════════════════════════════════════════════════════

def _call_agent(system_prompt, user_data, model=None, parse_json=True,
                web_search_max_uses=0, max_tokens=None):
    ...
    create_kwargs = {
        "model": model or AGENT_MODEL_TEAM,
    ...
    try:
        ...
        err = str(e)
        if "web_search" in err.lower() and web_search_max_uses > 0:
            logger.warning(f"web_search 사용 불가, 도구 제거 후 재시도: {e}")
            create_kwargs.pop("tools", None)
            response = client.messages.create(**create_kwargs)
        elif "rate_limit" in err.lower() or "429" in err:
            logger.error(f"⛔ Rate limit, 재시도 안 함: {err[:200]}")
            raise
        else:
            raise
    ...
    if usage:
        ...
        if cr > 0:
            saved = round(cr / max(1, cr + in_t) * 100)
            logger.info(f"📊 토큰: in={in_t} (cache={cr}, ~{saved}%) + out={out_t}")
        elif cc > 0:
    ...
    if search_count > 0:
        logger.info(f"🔍 웹 검색: {search_count}회")
...
def run_deep_analysis_agent(classification, holdings_data):
    ...
    deep = classification.get("deep_analysis", {}) if isinstance(classification, dict) else {}
    if not any([deep.get("buy_candidates"), deep.get("sell_review"), deep.get("holding_caution")]):
        logger.info("[Agent 2] 깊은 분석 대상 없음, skip")
        return {"analyses": []}

    h_map  = {h["ticker"]: h for h in holdings_data.get("holdings", []) if h.get("ticker")}
    dc_map = {d["ticker"]: d for d in holdings_data.get("double_confirm", []) if d.get("ticker")}

    def enrich(group_items, source_map):
        result = []
        for item in (group_items or []):
            tk = item.get("ticker")
            if not tk: continue
            base = source_map.get(tk, {"ticker": tk})
            result.append({**base, "group_reason": item.get("reason")})
        return result

    groups = {
        "buy_candidates":  enrich(deep.get("buy_candidates"),   dc_map),
        "sell_review":     enrich(deep.get("sell_review"),      h_map),
        "holding_caution": enrich(deep.get("holding_caution"),  h_map),
    }

    data = {
        "groups": groups,
        "market_context": classification.get("market_context"),
        "rules": {"stop_loss_pct": -15, "take_profit_pct": 50}
    }

    # 검색: 매수후보당 2, 매도검토당 1, 보유주의당 1 → 최대 10회
    n_buy  = len(groups["buy_candidates"])
    n_sell = len(groups["sell_review"])
    n_caut = len(groups["holding_caution"])
    max_search = min(n_buy * 2 + n_sell + n_caut, 10)

    return _call_agent(DEEP_ANALYSIS_PROMPT, data, parse_json=True,
                       web_search_max_uses=max_search,
                       max_tokens=AGENT_MAX_TOKENS_DEEP)

def run_summary_agent(classification, deep_analysis, holdings_data):
    """Agent 3: 부장 종합 마크다운 (검색 없음)"""
    data = {
        "market_context":    classification.get("market_context") if isinstance(classification, dict) else None,
        "market_color":      classification.get("market_color") if isinstance(classification, dict) else None,
        "shallow_check":     classification.get("shallow_check") if isinstance(classification, dict) else {},
        "analyses":          deep_analysis.get("analyses", []) if isinstance(deep_analysis, dict) else [],
        "portfolio_summary": holdings_data.get("portfolio_summary", {}),
        "cash":              holdings_data.get("cash", {}),
        "rules":             {"stop_loss_pct": -15, "take_profit_pct": 50},
        "analysis_time_kst": datetime.now(pytz.timezone("Asia/Seoul")).strftime("%Y-%m-%d %H:%M")
    }
    return _call_agent(SUMMARY_PROMPT, data, model=AGENT_MODEL_SUMMARY,
                       parse_json=False, web_search_max_uses=0,
                       max_tokens=AGENT_MAX_TOKENS_SUMMARY)
...
def _run_full_analysis_impl():
    ...
    try:
        ...
        holdings_data = _collect_holdings_data()
    except Exception as e:
    ...
    try:
        ...
        shallow_cnt = len(((classification.get("shallow_check") or {}).get("holding_normal", []))) if isinstance(classification, dict) else 0
        logger.info(f"[Agent 1] 분류 완료: deep={deep_cnt}, shallow={shallow_cnt}")
    except Exception as e:
    ...
    # 2. Agent 2: 깊은 분석
    try:
        deep_analysis = run_deep_analysis_agent(classification, holdings_data)
        logger.info(f"[Agent 2] 깊은 분석 완료: {len(deep_analysis.get('analyses', []))}개")
    except Exception as e:
        logger.error(f"[Agent 2] 실패: {e}")
        deep_analysis = {"error": str(e), "analyses": []}
        errors.append(f"deep_analysis: {e}")
    logger.info("⏸ 8초 대기")
    _time.sleep(8)

    # 3. Agent 3: 종합
    try:
        summary_text = run_summary_agent(classification, deep_analysis, holdings_data)
        logger.info("[Agent 3] 종합 완료")
    except Exception as e:
        logger.error(f"[Agent 3] 실패: {e}")
        summary_text = f"# 종합 보고 실패\n\n에러: {e}\n\n앞 단계 결과는 별도로 확인 가능합니다."
    ...
    if DATABASE_URL:
        ...
        try:
            conn = get_conn(); cur = conn.cursor()
            summary_t = summary_text if isinstance(summary_text, str) else json.dumps(summary_text, ensure_ascii=False)
            cur.execute("""
                INSERT INTO agent_analysis_cache
                (analysis_time, classification_result, deep_analysis_result, summary_text, error_log)
                VALUES (%s, %s, %s, %s, %s)
            """, (
                analysis_time,
                Json(sanitize(classification)),
                Json(sanitize(deep_analysis)),
                summary_t,
    ...
    logger.info(f"[3에이전트] 종합 분석 완료 ({duration:.1f}초)")
...
@limiter.limit("3/minute")
def trigger_full_analysis(request: Request, force: bool = False):
    """3에이전트 종합 분석. force=true는 5분 캐시 우회."""
    if not ANTHROPIC_AVAILABLE:
        return JSONResponse(status_code=500, content={"error": "anthropic 패키지 미설치"})
    if not ANTHROPIC_API_KEY:
        return JSONResponse(status_code=500, content={"error": "ANTHROPIC_API_KEY 미설정"})
    ...
    if _analysis_in_progress["running"]:
        ...
        if elapsed < 300:
            return JSONResponse(status_code=409, content={
                "error": "분석 진행 중",
                "message": f"이전 분석이 {elapsed:.0f}초 전에 시작되어 진행 중입니다. 잠시 후 재시도하세요.",
                "in_progress": True,
                "elapsed_sec": round(elapsed, 1)
            })

    # 5분 캐시 (force=true면 우회)
    if DATABASE_URL and not force:
        try:
            conn = get_conn(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            cur.execute("""
            ...
            if recent:
                ...
                logger.info(f"📦 5분 캐시 적중 ({elapsed_min:.1f}분 전) — API 호출 없음")
                return safe_json({
                    "analysis_time": recent["analysis_time"].isoformat() if recent.get("analysis_time") else None,
                    "classification": recent.get("classification_result"),
                    "deep_analysis": recent.get("deep_analysis_result"),
...
def get_latest_analysis(request: Request):
    ...
    try:
        ...
        return safe_json({
            ...
            "deep_analysis": row.get("deep_analysis_result"),
